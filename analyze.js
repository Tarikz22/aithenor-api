...
export default async function handler(req, res) {
  ...
  try {
    ...
    workbook.SheetNames.forEach((sheetName, index) => {
      ...
      if (rows.length === 0) {
        allData += 'No data found in this tab.\n';
        return;
      }

      // Write each row as labeled key:value pairs
...
Below is the complete data from all 5 tabs of this hotel's Excel file. Each row is labeled with column names so you can read it accurately:
...
From the STR Daily Report tab, check the MPI (Index) and ARI (Index) columns. If the majority of rows show MPI < 100 AND ARI > 100, the trigger is met. If trigger not met, output: [{"trigger_met": false}] and stop.
